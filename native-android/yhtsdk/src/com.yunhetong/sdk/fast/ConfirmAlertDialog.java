...
public class ConfirmAlertDialog extends AlertDialog implements
    ...
    private void init(Context context) {
        int resId = YhtResourceFinder.findContentView(context, "yht_dialog_confirm_alert");
//        int resId = R.layout.yht_dialog_confirm_alert;
        View ui = LayoutInflater.from(context).inflate(resId, null);
        this.setView(ui);
//        confirm_alert_msg = (TextView) ui.findViewById(R.id.yht_confirm_alert_msg);
        confirm_alert_msg = (TextView) ui.findViewById(YhtResourceFinder.findView(context, "yht_confirm_alert_msg"));
//        cancelBt = (Button) ui.findViewById(R.id.yht_confirm_alert_cancel);
        cancelBt = (Button) ui.findViewById(YhtResourceFinder.findView(context, "yht_confirm_alert_cancel"));
//        sureBt = (Button) ui.findViewById(R.id.yht_confirm_alert_sure);
        sureBt = (Button) ui.findViewById(YhtResourceFinder.findView(context, "yht_confirm_alert_sure"));
    ...
    public void onClick(View v) {
        int id = v.getId();
//        int resId = R.id.yht_confirm_alert_cancel;
//        int resId2 = R.id.yht_confirm_alert_sure;
        if (id == YhtResourceFinder.findView(this.getContext(), "yht_confirm_alert_cancel")) {
            if (ls != null) {
                ls.ComfireAlertDialog2Cancel(targetId, this);
            }
        } else if (id == YhtResourceFinder.findView(getContext(), "yht_confirm_alert_sure")) {
            if (ls != null) {
